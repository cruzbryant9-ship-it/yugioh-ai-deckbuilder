...
def evaluate_matchup_matrix_update(
    ...
    if previous_side_score and side_score < previous_side_score - config.max_side_deck_score_drop:
    ...
    if previous_coverage and coverage < previous_coverage - config.max_matchup_coverage_drop:
    ...
    if previous_resilience and resilience < previous_resilience - config.max_resilience_score_drop:
    ...
    if previous_post_side_score and post_side_score < previous_post_side_score - config.max_post_side_score_drop:
    ...
    if _number(matrix_summary.get("average_post_side_delta")) < config.min_post_side_delta:
        reasons.append("matrix post-side delta is strongly negative")
    if _number(matrix_summary.get("post_side_valid_rate")) < config.min_post_side_valid_rate:
        reasons.append("matrix side plans are invalid too often")
    if _number(matrix_summary.get("average_valid_candidate_rate")) < config.min_valid_candidate_rate:
        reasons.append("matrix valid side-plan candidate rate is too low")
    if _number(matrix_summary.get("side_optimization_success_rate")) < config.min_side_optimization_success_rate:
        reasons.append("matrix side optimization fails too often")
    if _number(matrix_summary.get("average_memory_aided_post_side_delta")) < config.min_memory_delta_difference:
...
def evaluate_training_batch(
    ...
    if previous_average and average_score < previous_average - config.max_average_score_drop:
        reasons.append(
    ...
    if previous_playable and playable_rate < previous_playable - config.max_playable_rate_drop:
        reasons.append(
    ...
    if previous_brick or brick_rate:
        if brick_rate > previous_brick + config.max_brick_rate_increase:
            reasons.append(
    ...
    if package_quality_score and package_quality_score < config.min_package_quality_score:
        reasons.append(
    ...
    if previous_matchup_coverage and matchup_coverage < previous_matchup_coverage - config.max_matchup_coverage_drop:
    ...
    if _number(summary.get("average_post_side_delta")) < config.min_post_side_delta:
        reasons.append("post-side delta is strongly negative")
    post_side_valid_rate = _number(summary.get("post_side_valid_rate"))
    if "post_side_valid_rate" in summary and post_side_valid_rate < config.min_post_side_valid_rate:
        reasons.append(f"side plan validity too low: {post_side_valid_rate} < {config.min_post_side_valid_rate}")
    if "average_valid_candidate_rate" in summary and _number(summary.get("average_valid_candidate_rate")) < config.min_valid_candidate_rate:
        reasons.append("valid side-plan candidate rate is too low")
    if "side_optimization_success_rate" in summary and _number(summary.get("side_optimization_success_rate")) < config.min_side_optimization_success_rate:
        reasons.append("side optimization fails too often")
    if "average_memory_post_side_delta_difference" in summary and _number(summary.get("average_memory_post_side_delta_difference")) < config.min_memory_delta_difference:
    ...
    if previous_choke_stop and choke_stop < previous_choke_stop - config.max_choke_stop_rate_drop:
    ...
    if previous_recovery and recovery > previous_recovery + config.max_opponent_recovery_rate_increase:
    ...
    if previous_poor and poor > previous_poor + config.max_poor_interruption_count_increase:
    ...
    if previous_timing and timing < previous_timing - config.max_timing_precision_drop:
    ...
    if previous_pivot and pivot > previous_pivot + config.max_pivot_risk_increase:
    ...
    if previous_backup and backup > previous_backup + config.max_backup_line_success_increase:
        reasons.append("backup line success rate rose too much")
    if _number(summary.get("average_late_interruption_risk")) > config.max_late_interruption_risk:
    ...
    if previous_graph_stop and graph_stop < previous_graph_stop - config.max_graph_stop_rate_drop:
    ...
    if previous_graph_pivot and graph_pivot > previous_graph_pivot + config.max_graph_pivot_rate_increase:
    ...
    if previous_graph_reduction and graph_reduction < previous_graph_reduction - config.max_graph_endboard_reduction_drop:
    ...
    if previous_graph_poor and graph_poor > previous_graph_poor + config.max_graph_poor_interruption_increase:
    ...
    if "average_opponent_resource_valid_rate" in summary and resource_valid < config.min_opponent_resource_valid_rate:
        reasons.append("opponent resource valid rate dropped below sanity threshold")
    if resource_failure > config.max_opponent_resource_failure_rate:
    ...
    if previous_best_line_score and best_line_score < previous_best_line_score - config.max_best_line_score_drop:
        reasons.append(
    ...
    if previous_graph_valid and graph_valid_rate < previous_graph_valid - config.max_graph_valid_line_rate_drop:
    ...
    if previous_graph_line_score and graph_line_score < previous_graph_line_score - config.max_graph_line_score_drop:
    ...
    if graph_failed_rate > previous_graph_failed + config.max_graph_failed_line_rate_increase:
    ...
    if graph_risk > previous_graph_risk + config.max_graph_risk_score_increase:
    ...
    if previous_resource_valid and resource_valid < previous_resource_valid - config.max_resource_valid_line_rate_drop:
    ...
    if no_valid_line_rate > previous_no_valid_line_rate + config.max_no_valid_line_rate_increase:
    ...
    if best_line_failure_rate > previous_best_line_failure_rate + config.max_best_line_failure_rate_increase:
    ...
    if previous_cost_condition_valid and cost_condition_valid < previous_cost_condition_valid - config.max_cost_condition_valid_rate_drop:
    ...
    for key, label in (
        ...
        current = _number(real_metrics.get(key))
        previous = _number(previous_real.get(key))
        if current > previous + config.max_cost_condition_failure_increase:
    ...
    if previous_branch_valid and branch_valid < previous_branch_valid - config.max_branch_valid_rate_drop:
    ...
    if no_valid_branch > previous_no_valid_branch + config.max_branch_failure_rate_increase:
    ...
    for key, label in (
        ...
        current = _number(real_metrics.get(key))
        previous = _number(previous_real.get(key))
        if current > previous + config.max_history_failure_rate_increase:
    ...
    if previous_resilience_score and resilience_score < previous_resilience_score - config.max_resilience_score_drop:
    ...
    if previous_interrupted_success and interrupted_success < previous_interrupted_success - config.max_interrupted_success_rate_drop:
    ...
    if interruption_risk > previous_interruption_risk + config.max_interruption_risk_increase:
    ...
    for key, label in (
        ...
        current = _number(real_metrics.get(key))
        previous = _number(previous_real.get(key))
        if current > previous + config.max_vulnerability_rate_increase:
    ...
    if previous_typed_valid and typed_valid < previous_typed_valid - config.max_typed_material_valid_rate_drop:
    ...
    for key, label in (
        ...
        current = _number(real_metrics.get(key))
        previous = _number(previous_real.get(key))
        if current > previous + config.max_typed_material_failure_rate_increase:
...
def evaluate_curated_opponent_memory_update(
    ...
    if no_memory_resilience and resilience < no_memory_resilience - config.max_resilience_score_drop:
    ...
    if _number(summary.get("average_choke_stop_rate", summary.get("choke_stop_rate"))) < 0 and summary:
        reasons.append("choke stop rate is invalid")
    if _number(summary.get("average_poor_interruption_count", summary.get("poor_interruption_count"))) > 20:
        reasons.append("post-side memory recommends poor interruptions too often")
    if _number(summary.get("average_late_interruption_risk", summary.get("late_interruption_risk"))) > config.max_late_interruption_risk:
